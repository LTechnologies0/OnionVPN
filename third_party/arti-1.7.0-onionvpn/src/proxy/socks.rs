...
/// OnionVPN: optional ExitNodes-country analogue for SOCKS streams
/// (`StreamPrefs::exit_country`). Set via [`set_onionvpn_exit_country`].
static ONIONVPN_EXIT_COUNTRY: std::sync::Mutex<Option<String>> = std::sync::Mutex::new(None);

/// Set (or clear) the global exit-country preference applied to new SOCKS streams.
///
/// OnionVPN Ext JNI writes a single ISO-3166 alpha-2 code (lowercase), matching
/// C Tor `ExitNodes {cc}` when only one country is selected. Multi-country lists
/// are not supported on the SOCKS path in arti-client 0.36.0.
#[cfg_attr(feature = "experimental-api", visibility::make(pub))]
pub(crate) fn set_onionvpn_exit_country(cc: Option<&str>) {
    if let Ok(mut g) = ONIONVPN_EXIT_COUNTRY.lock() {
        *g = cc
            .map(str::trim)
            .filter(|s| !s.is_empty())
...
fn onionvpn_exit_country() -> Option<String> {
    ONIONVPN_EXIT_COUNTRY.lock().ok().and_then(|g| g.clone())
}
...
fn stream_preference(req: &SocksRequest, addr: &str) -> StreamPrefs {
    ...
    // OnionVPN ExitNodes country → StreamPrefs::exit_country (requires geoip).
    #[cfg(feature = "geoip")]
    if let Some(cc) = onionvpn_exit_country() {
        if let Ok(code) = cc.parse::<arti_client::CountryCode>() {
            prefs.exit_country(code);
        } else {
            warn!("OnionVPN exit country '{cc}' is not a valid CountryCode");
        }
    }
    prefs
}
