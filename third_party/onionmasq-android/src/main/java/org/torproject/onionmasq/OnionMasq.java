...
import static org.torproject.onionmasq.BridgeLineParser.VANILLA;
...
public class OnionMasq {
    ...
    static int getConnectionOwnerUid(int protocol, byte[] rawSourceAddress, int sourcePort, byte[] rawDestinationAddress, int destinationPort) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            } catch (UnknownHostException | IllegalStateException | SecurityException e) {
                e.printStackTrace();
            }
        }
    ...
    public static synchronized void start(int fileDescriptor, String bridgeLines) throws OnionmasqException {
        try {
            ...
            boolean iptProxyStarted = startIPtProxy(config);
            OnionMasqJni.runProxy(
                    ...
                    getInstance().appContext.getFilesDir().getAbsolutePath(),
                    iptProxyStarted ? config.getBridgeType() : null,
                    iptProxyStarted ? config.getPtClientPort() : 0,
    ...
    /**
     * Starts IPtProxy with a given IPtConfig.
     *
     * @param iPtConfig bridge line configuration
     * @return true if IPtProxy could be started, false if starting IPtProxy was not required to handle the bridge config or the config was invalid.
     * @throws Exception in case starting IPtProxy failed
     */
    private static boolean startIPtProxy(BridgeLineParser.IPtConfig iPtConfig) throws Exception {
        synchronized (BRIDGE_CONFIG_LOCK) {
            ...
            if (iPtConfig.getBridgeType().equals(VANILLA)) {
                Log.d(TAG, "Using a vanilla bridge, no need to start IPtProxy");
                return false;
            }
    ...
    private Controller initializeController(Context context)  {
        ...
        return IPtProxy.newController(ptDir.getAbsolutePath(), true, false, "DEBUG", new OnTransportEvents() {
            ...
            @Override public void error(String name, java.lang.Exception error) {
                Log.e(TAG, "IPtProxy transport: " + name + " error: " + error);
            }
            @Override public void stopped(String name, java.lang.Exception error) {
     ...
     * See <a href="https://github.com/tladesignz/IPtProxy/blob/5.4.2/IPtProxy.go/controller.go#L143">Controller documentation</a>
     *
