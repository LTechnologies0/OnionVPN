...
public class LogObservable extends ViewModel {
    ...
    public void setVerboseLogs(boolean verbose) {
        synchronized (LOCK) {
            if (this.verboseLogs == verbose) {
                return;
            }
            this.verboseLogs = verbose;
            // force an update signal for the observers in case the current list is not empty
            LinkedList<LogItem> list = logListData.getValue();
            if (list == null || list.isEmpty()) {
                return;
            }
    ...
    public void addLog(String log, boolean verbose) {
        synchronized (LOCK) {
            ...
            if (reverseOrder) {
                list.addFirst(new LogItem(System.currentTimeMillis(), log, verbose));
                if (list.size() > maxCapacity) {
                    list.removeLast();
                }
            } else {
                list.add(new LogItem(System.currentTimeMillis(), log, verbose));
                if (list.size() > maxCapacity) {
                    list.removeFirst();
                }
            }
    ...
    public LinkedList<LogItem> getLogList() {
        synchronized (LOCK) {
            ...
            LinkedList<LogItem> newList = new LinkedList<>(logItemList);
            if (verboseLogs) {
                return newList;
            }
            newList.removeIf(logItem -> logItem.verbose);
            return newList;
        }
    }
