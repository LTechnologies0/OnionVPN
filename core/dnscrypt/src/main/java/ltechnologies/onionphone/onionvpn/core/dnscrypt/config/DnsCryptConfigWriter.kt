...
object DnsCryptConfigWriter {
    ...
    fun write(
        ...
        val resolved = DnsCryptPublicResolvers.resolveName(
            serverName.ifBlank { preferences.dnsCryptServerName },
        )
        val serverNamesBlock = if (resolved == DnsCryptPublicResolvers.AUTO) {
            "# server_names omitted — use every resolver matching require_* filters"
        } else {
            "server_names = ['$resolved']"
        }
        val staticBlock = if (resolved == DnsCryptPublicResolvers.AUTO) {
            ""
        } else {
            """
            [static]
              [static.'$resolved']
                stamp = '${stampFor(resolved)}'
            """.trimIndent()
        }
        return """
