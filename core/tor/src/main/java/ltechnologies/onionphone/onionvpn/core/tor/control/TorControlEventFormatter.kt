...
object TorControlEventFormatter {
    /** One-line CTRL … representation of [event] (fingerprints truncated for OPSEC). */
    fun format(event: TorControlEvent): String = redactTorFingerprints(
        when (event) {
    ...
    /** Truncate `$HEX40` / long hex blobs so log buffers do not dump full relay identities. */
    fun redactTorFingerprints(raw: String): String =
